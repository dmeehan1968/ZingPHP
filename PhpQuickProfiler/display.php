...
if($output['queryTotals']['count'] ==  0) {
	...
	$duplicates = array_diff_assoc($queries, array_unique($queries));
	echo '<table class="side" cellspacing="0">
		  ...
		  <tr><td><var>'.count($duplicates).'</var> <h4>Duplicates</h4></td></tr>
		 </table>
		<table class="main" cellspacing="0">';
		foreach ($duplicates as $index => $duplicate) {
			echo '<tr><td>Duplicate: ['.$index.'] '.$duplicate.'</td></tr>';
		}
		$class = '';
		foreach($output['queries'] as $index => $query) {
			echo '<tr>
				<td class="'.$class.'">['.$index.'] '.$query['time'].' - '.$query['sql'];
			if($query['explain']) {
